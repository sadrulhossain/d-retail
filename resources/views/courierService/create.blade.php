...
<div class="col-md-12">
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-user"></i>@lang('label.CREATE_COURIER_SERVICE_PROVIDER')
            </div>
        </div>
        <div class="portlet-body form">
            {!! Form::open(array('group' => 'form', 'class' => 'form-horizontal', 'files' => true,'id' => 'courierServiceForm')) !!}
            {!! Form::hidden('filter', Helper::queryPageStr($qpArr)) !!}
            {{csrf_field()}}
            <div class="form-body">
                <div class="row">
                    <h3 class="form-section title-section bold">@lang('label.COURIER_SERVICE_PROVIDER_INFORMATION')</h3>
                    <div class="col-md-offset-1 col-md-7">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                {{ Form::textarea('address', null, ['id'=> 'address', 'class' => 'form-control','size' => '30x2','autocomplete' => 'off']) }}
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("click", ".add-contact-person", function () {
            $.ajax({
                url: "{{ route('courierService.createContactPerson') }}",
                type: "POST",
        ...
        $(document).on("click", "#submitBuyer", function (e) {
            ...
            var formData = new FormData($('#courierServiceForm')[0]);

            $.ajax({
                url: "{{ route('courierService.store') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
        ...
        $(document).on("click", ".add-phone-number", function () {
            ...
            $.ajax({
                url: "{{ URL::to('courierService/addPhoneNumber')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

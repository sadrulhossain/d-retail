        ...
        <div class="table-responsive col-md-12 webkit-scrollbar" style="max-height: 600px;">
            <table class="table table-bordered table-hover module-access-view" id="dataTable">
                ...
                <tbody class="access-check">
                    ...
                    $moduleInvisible = 0;
                    if (in_array($moduleId, [52, 53, 54, 55, 56, 57, 103]) && !in_array($groupId, [1, 12, 15])) {
                        $moduleInvisible = 1;
                    }
                    if (in_array($moduleId, [108]) && !in_array($groupId, [1, 11, 12])) {
                        $moduleInvisible = 1;
                    }
                    if (in_array($moduleId, [118]) && !in_array($groupId, [1, 11])) {
                        $moduleInvisible = 1;
                    }
                    ?>
                    @if($moduleInvisible == 0)
                    <tr>
...
<div class="form-actions">
    <div class="row">
        <div class="col-md-offset-3 col-md-8">
            ...
            @if(!$userGroupToAccessArr->isEmpty() && $groupId != 1 && !empty($userAccessArr[3][4]))
            ...
            @endif
            <a href="{{ URL::to('/admin/aclUserGroupToAccess/userGroupToAccess') }}" class="btn btn-circle btn-outline grey-salsa">@lang('label.CANCEL')</a>
        </div>
    </div>
</div>

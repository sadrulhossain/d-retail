...
<script type="text/javascript">
    $(function () {
                ...
                closeOnCancel: false},
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                url: "{{URL::to('admin/aclUserGroupToAccess/relateUserGroupToAccess')}}",
                                type: "POST",
        ...
        $(document).on('click', '#btnRevoke', function () {
            ...
            $.ajax({
                url: "{{ URL::to('admin/aclUserGroupToAccess/getUserGroupListToRevoke') }}",
                type: 'POST',
                dataType: 'json',
                headers: {
                ...
                closeOnCancel: false},
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                url: "{{URL::to('admin/aclUserGroupToAccess/revokeUserGroupAccess')}}",
                                type: "POST",
                                dataType: 'json', // what to expect back from the PHP script, if anything
                                ...
                                beforeSend: function () {
                                    $('#btnRevokeFinalize').prop('disabled', true);
                                    App.blockUI({boxed: true});
                                },
                                success: function (res) {
                                    toastr.success(res.message, res.heading, options);
                                ...
                                error: function (jqXhr, ajaxOptions, thrownError) {
                                    ...
                                    $('#btnRevokeFinalize').prop('disabled', false);
                                    App.unblockUI();
                                }
                            });
                        } else {
    ...
    function loadAccessControl() {
        ...
        $.ajax({
            url: '{{URL::to("admin/aclUserGroupToAccess/getAccessControl/")}}',
            type: 'POST',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

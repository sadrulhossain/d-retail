                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center vcenter width-80">
                                        {!! Form::text('customer_demand['.$target->id.']', !empty($target->customer_demand) ? $target->customer_demand : null, ['id'=> 'customerDemand_'.$target->id, 'data-id' => $target->id, 'class' => 'form-control width-inherit text-right integer-decimal-only customer-demand']) !!}
                                    </td>
                                    <td class="text-center vcenter width-80">
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('keyup', '.product-quantity', function () {
            ...
            if (productTotalPriceVal == '' || isNaN(productTotalPriceVal)) {
            ...
            if (quantity > availableQty) {
                swal({
                    title: '@lang("label.GIVEN_QUANTITY_IS_GREATER_THAN_AVAILABLE_QUANTITY")',
                    confirmButtonColor: '#DD6B55',
                    confirmButtonText: 'Ok',
                    closeOnConfirm: true,
                }, function (isConfirm) {
                    grandTotalPriceVal = grandTotalPriceVal - productTotalPriceVal;
                    $("#productQuantity_" + dataId).val('');
                    $("#productTotalPrice_" + dataId).val('');
                    $("#grandTotalPrice").val(parseFloat(grandTotalPriceVal).toFixed(2));
                    setTimeout(function () {
                        $("#productQuantity_" + dataId).focus();
                    }, 250);

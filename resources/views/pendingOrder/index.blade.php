...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(!empty($userAccessArr[52][2]) && Auth::user()->group_id == 14)
                <!--                <a class="btn btn-default btn-sm create-new" href="{{ URL::to('admin/pendingOrder/create'.Helper::queryPageStr($qpArr)) }}"> @lang('label.CREATE_NEW_ORDER')
        ...
        <div class="portlet-body">
            <!-- Begin Filter-->
            {!! Form::open(array('group' => 'form', 'url' => 'admin/pendingOrder/filter','class' => 'form-horizontal')) !!}
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("click", ".start-processing", function () {
            ...
            if (id) {
                $.ajax({
                    url: "{{URL::to('admin/pendingOrder/startProcessing')}}",
                    type: 'POST',
                    dataType: 'json',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", ".cancel-order", function () {
            ...
            swal({
                ...
                if (id) {
                    $.ajax({
                        url: "{{URL::to('admin/pendingOrder/cancel')}}",
                        type: 'POST',
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(".view-order-details").on("click", function (e) {
            ...
            var id = $(this).attr("data-id");
            $.ajax({
                url: "{{ URL::to('/admin/pendingOrder/viewStockDemand')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

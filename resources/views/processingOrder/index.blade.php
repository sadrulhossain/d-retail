...
//                            }
                            ...
                            <td class="vcenter text-center"> 
                                {{ number_format($dueQty) }}
                            </td>
                            <td class="vcenter text-right text-{{$text2}}"> 
                                {{ !empty($availabelStock) ? number_format($availabelStock, 0) : 0 }} 
                            </td>
                            ...
                            <td class="td-actions text-center vcenter " rowspan="{{!empty($order['products']) ? sizeof($order['products']) : 1}}">
                                <div class="width-inherit">
                                    ...
                                    @if(!empty($userAccessArr[103][14]))
                                    @if(!empty($deliveryArr))
                                    @if( array_key_exists($order['order_id'],$deliveryArr) )
                                    <button class="btn btn-xs tooltips green-jungle vcenter mark-delivered-btn" data-id="{!! $order['order_id'] !!}"  title="@lang('label.MARK_AS_DELIVERED')">
                                    ...
                                    <button class="btn btn-xs blue-sharp tooltips vcenter view-order-details" data-id="{!! $order['order_id'] !!}" href="#modalViewStockDemand"  data-toggle="modal" title="@lang('label.VIEW_STOCK_DEMAND')">
                                    ...
                                    <button class="btn btn-xs {{$btnClass}} tooltips vcenter" data-id="{!! $order['order_id'] !!}" {{$btnHref}}  data-toggle="modal"  title="{{$btnTitle}}">
                                        <i class="fa fa-cart-plus"></i>
                                    </button>
                                    @endif
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', ".mark-delivered-btn", function (e) {
            ...
            swal({
                ...
                confirmButtonText: "@lang('label.MARK_AS_DELIVERED')",
                closeOnConfirm: true,
                closeOnCancel: true,
            }, function (isConfirm) {
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: "{{ URL::to('admin/processingOrder/markAsDelivered')}}",
                        type: "POST",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
        ...
        $(document).on('click', ".delivery-details", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('admin/processingOrder/getDeliveryDetails')}}",
                type: "POST",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },

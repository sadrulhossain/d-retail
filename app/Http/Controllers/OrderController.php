...
class OrderController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if (Auth::user()->group_id == 12) {
            ...
            $whList = WarehouseToSr::where('sr_id', Auth::user()->id)->pluck('warehouse_id', 'warehouse_id')->toArray();
        }
        ...
        $orderNoList = Order::whereIn('status', ['0']);
...
//        
        ...
        return view('order.index')->with(compact('request', 'qpArr', 'orderArr', 'orderNoList', 'targetArr', 'retailerList', 'srList'));
    }
    ...
    public function filter(Request $request) {
        ...
        return Redirect::to('admin/order?' . $url);
    }
    ...
    public function create(Request $request) {
        ...
        $targetArr = WarehouseStore::join('product_sku_code', 'product_sku_code.id', '=', 'wh_store.sku_id')
                        ...
                        ->select('product_sku_code.sku', 'product_sku_code.id'
                                ...
                                , 'wh_store.quantity as available_quantity','wh_store.warehouse_id')->get();

        return view('order.create')->with(compact('qpArr', 'creationDate', 'srList', 'retailerList', 'targetArr', 'warehouse'));
    }

    public function retailerWisePrice(Request $request) {
        //
        $warehouse = WarehouseToSr::join('warehouse', 'warehouse.id', 'warehouse_to_sr.warehouse_id')
                ->where('warehouse_to_sr.sr_id', Auth::user()->id)
                ->select('warehouse_to_sr.warehouse_id', 'warehouse.name as warehouse_name')
        ...
        $retailer = Retailer::find($retailer_id)->where('approval_status', '1');

        if (!empty($retailer->type)) {
            if ($retailer->type == '1') {
                $targetArr = WarehouseStore::join('product_sku_code', 'product_sku_code.id', '=', 'wh_store.sku_id')
                                ->where('wh_store.warehouse_id', $warehouseId)
                                ->select('product_sku_code.sku', 'product_sku_code.id'
                                        , 'product_sku_code.selling_price', 'product_sku_code.distributor_price', 'product_sku_code.product_id'
                                        , 'wh_store.quantity as available_quantity')->get();
            } elseif ($retailer->type == '2') {
                $targetArr = WarehouseStore::join('product_sku_code', 'product_sku_code.id', '=', 'wh_store.sku_id')
                                ->where('wh_store.warehouse_id', $warehouseId)
                                ->select('product_sku_code.sku', 'product_sku_code.id'
                                        , 'product_sku_code.distributor_price as selling_price', 'product_sku_code.product_id'
                                        , 'wh_store.quantity as available_quantity')->get();
            }
            $view = view('order.showRetailerWisePrice', compact('targetArr', 'warehouse'))->render();
...
//        exit;
    ...
    public function viewStockDemand(Request $request) {
        $loadView = 'order.showStockDemand';
    ...
    public function store(Request $request) {
        ...
        $message['customer_demand.*.required'] = __('label.CUSTOMER_FIELD_IS_REQUIRED');
        $message['product_quantity.*.required'] = __('label.PRODUCT_QUANTITY_FIELD_IS_REQUIRED');
...
//        exit();
        ...
        return view('order.edit')->with(compact('qpArr', 'retailerList', 'targetArr', 'creationDate', 'orderDetails'));
    }

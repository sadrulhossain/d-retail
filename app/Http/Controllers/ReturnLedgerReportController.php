...
class ReturnLedgerReportController extends Controller {
...
//            $previousDelivery['amount'] = !empty($prevDeliveryInfo->total_amount) ? $prevDeliveryInfo->total_amount : 0;
            ...
            $previousBalance['quantity'] = $previousReturn['quantity'];
            $previousBalance['amount'] = $previousReturn['amount'];
        ...
        if ($request->view == 'print') {
            ...
            return view('report.returnLedger.print.index')->with(compact('request', 'targetArr', 'totalAmount'
            ...
            $pdf = PDF::loadView('report.returnLedger.print.index', compact('request', 'targetArr', 'totalAmount'
                                    , 'productList', 'konitaInfo', 'phoneNumber', 'fromDate', 'toDate', 'totalBalance'
                                    , 'totalDelivery', 'totalReturn', 'totalDamage', 'totalCheckIn', 'balanceArr'
                                    , 'ledgerArr', 'previousBalance'))
                    ->setPaper('a3', 'portrait')
                    ->setOptions(['defaultFont' => 'sans-serif']);
            return $pdf->download('return_ledger_report.pdf');
//            return $pdf->stream();
        } else {
            return view('report.returnLedger.index')->with(compact('request', 'targetArr', 'totalAmount', 'productList'
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('admin/returnLedgerReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }


        return Redirect::to('admin/returnLedgerReport?generate=true&' . $url);
    }

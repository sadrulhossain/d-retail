...
class ProductCategoryController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $qpArr = $request->all();
        $targetArr = ProductCategory::select('*')->orderBy('order', 'asc');
        ...
        if ($targetArr->isEmpty() && isset($qpArr['page']) && ($qpArr['page'] > 1)) {
            $page = ($qpArr['page'] - 1);
            return redirect('/admin/productCategory?page=' . $page);
        }
    ...
    public function store(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            // return redirect('admin/productCategory/create' . $pageNumber)
            //                 ->withInput()
        ...
        $target->highlighted = !empty($request->highlighted) ? $request->highlighted : '0';
        if ($request->hasFile('image')) {
            $img = $request->file('image');
            $full_name = uniqid() . '.' . $img->getClientOriginalExtension();
            $path = "public/uploads/category/";
            $img->move($path, $full_name);
            $target->image = $full_name;
        }

        if ($target->save()) {
            Helper::insertOrder($this->controller, $request->order, $target->id);
            // Session::flash('success', __('label.PRODUCT_CATEGORY_CREATED_SUCCESSFULLY'));
            return Response::json(array('heading' => 'Success', 'message' => __('label.PRODUCT_CATEGORY_CREATED_SUCCESSFULLY')), 200);
        } else {
            // Session::flash('error', __('label.PRODUCT_CATEGORY_COULD_NOT_BE_CREATED'));
            return Response::json(array('success' => false, 'message' => __('label.PRODUCT_CATEGORY_COULD_NOT_BE_CREATED')), 401);
        }
    }
    ...
    public function update(Request $request) { //print_r($request->all());exit;
        $target = ProductCategory::find($request->id);
        ...
        $validator = Validator::make($request->all(), [
                    'name' => 'required|unique:product_category,name,' . $request->id,
                    'code' => 'required|unique:product_category,code,' . $request->id,
                    'order' => 'required|not_in:0',
                    'description' => 'required',
        ]);
        ...
        if ($validator->fails()) {
            return Response::json(array('success' => false, 'heading' => 'Validation Error', 'message' => $validator->errors()), 400);
            // return redirect('admin/productCategory/' . $id . '/edit' . $pageNumber)
            //                 ->withInput()
            //                 ->withErrors($validator);
        }
        if(empty($target)){
        ...
        if ($request->hasFile('image')) {
            ...
            if (!empty($target->image)) {
                File::delete('public/uploads/category/' . $target->image);
            }
            $img->move($path, $full_name);
            $target->image = $full_name;
        }
        if ($target->save()) {
            ...
            return Response::json(array('heading' => 'Success', 'message' => __('label.PRODUCT_CATEGORY_UPDATED_SUCCESSFULLY')), 200);
        } else {
            // Session::flash('error', __('label.PRODUCT_CATEGORY_COULD_NOT_BE_UPDATED'));
            return Response::json(array('success' => false, 'message' => __('label.PRODUCT_CATEGORY_COULD_NOT_BE_UPDATED')), 401);
        }
    }

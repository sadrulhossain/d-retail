...
class CancelledOrderController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $orderNoList = Order::whereIn('status', ['8']);
        ...
        //inquiry Details
        $targetArr = Order::where('order.status', '8')
        ...
        $targetArr = $targetArr->select(DB::raw("CONCAT(users.first_name,' ',users.last_name) AS user_name")
                        , 'order.id as order_id', 'order.order_no', 'order.grand_total', 'order.status'
        ...
        if ($targetArr->isEmpty() && isset($qpArr['page']) && ($qpArr['page'] > 1)) {
            $page = ($qpArr['page'] - 1);
            return redirect('/admin/cancelledOrder?page=' . $page);
        }
        ...
        $orderDetailArr = OrderDetails::join('order', 'order.id', 'order_details.order_id')
                ...
                ->select('order_details.*', 'product_sku_code.sku'
                        , 'brand.name as brand_name', 'product.name as product_name'
                        , 'product_sku_code.available_quantity', 'retailer.name as retailer_name'
                        , 'product_sku_code.attribute')
                ->get();
...
//        print_r($orderArr);
//        exit;
        return view('cancelledOrder.index')->with(compact('request', 'qpArr', 'orderArr', 'targetArr', 'orderNoList', 'retailerList', 'srList'));
    }
    ...
    public function filter(Request $request) {
        ...
        return Redirect::to('admin/cancelledOrder?' . $url);
    }

...
class DashboardController extends Controller {
    ...
    public function index(Request $request) {
        if (in_array(Auth::user()->group_id, [9, 18, 19])) {
            return redirect('/');
        }
...
//        echo '<pre>';        print_r($totalSellAmountArr);exit;
        ...
        $pmModeWiseTodaysCollectionArr = OrderDetails::join('order', 'order.id', 'order_details.order_id')
                ->select(DB::raw('SUM(order_details.total_price) as total_price'), 'order.payment_collection')
                ->groupBy('order.payment_collection')
                ->whereDate('created_at', '=', $toDayDate)
                ->pluck('total_price', 'order.payment_collection')
                ->toArray();
...
//        exit;
        ...
        if (Auth::user()->group_id == 12) {
            $warehouseId = WhToLocalWhManager::select('warehouse_id')
                    ...
                    ->first();
            $pendingOrderArr = $pendingOrderArr->where('order.warehouse_id', $warehouseId);
        }
...
//        
        ...
        return view('admin.dashboard')->with(compact('request', 'lastOneYearExpense', 'lastOneYearEarning'
                                ...
                                , 'paymentModeList', 'threeMnthsOrderByQtyFrSrArr', 'srArr', 'threeMnthsOrderByAmountForSrArr', 'pendingOrder'));
    }

...
class CustomerLedgerReportController extends Controller {
    ...
    public function index(Request $request) {
        $customerList = ['0' => __('label.SELECT_CUSTOMER_OPT')] + Customer::select(DB::raw("CONCAT(name, ' (', username, ')') as customer_name"), 'id')
                        ->pluck('customer_name', 'id')->toArray();

        $fromDate = $toDate = '';
        $totalAmount = 0;
        $targetArr = $customerCheckInInfo = $ledgerArr = $previousBalance = [];
...
//
            ...
            $previousBalance['quantity'] = $previousDelivery['quantity'];
            $previousBalance['amount'] = $previousDelivery['amount'];
        ...
        if ($request->view == 'print') {
            ...
            return view('report.customerLedger.print.index')->with(compact('request', 'targetArr', 'totalAmount'
                                    , 'customerList', 'konitaInfo', 'phoneNumber', 'fromDate', 'toDate', 'totalBalance'
            ...
            $pdf = PDF::loadView('report.customerLedger.print.index', compact('request', 'targetArr', 'totalAmount'
                                    , 'customerList', 'konitaInfo', 'phoneNumber', 'fromDate', 'toDate', 'totalBalance'
                                    , 'totalDelivery', 'totalReturn', 'totalDamage', 'totalCheckIn', 'balanceArr'
                                    , 'ledgerArr', 'previousBalance'))
                    ->setPaper('a3', 'portrait')
                    ->setOptions(['defaultFont' => 'sans-serif']);
            return $pdf->download('customer_ledger_report.pdf');
//            return $pdf->stream();
        } else {
            return view('report.customerLedger.index')->with(compact('request', 'targetArr', 'totalAmount', 'customerList'
    ...
    public function filter(Request $request) {
        ...
        $url = 'customer_id=' . $request->customer_id . '&from_date=' . $request->from_date . '&to_date=' . $request->to_date;
        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect('admin/customerLedgerReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }


        return Redirect::to('admin/customerLedgerReport?generate=true&' . $url);
    }

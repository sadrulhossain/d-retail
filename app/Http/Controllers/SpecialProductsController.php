...
class SpecialProductsController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $prevData = ProductOffer::where('category', 3)->select('sku_data')->first();
        $prevSku = [];
        if (!empty($prevData)) {
...
//        exit;
        ...
        return view('specialProducts.index')->with(compact('request', 'qpArr', 'targetArr', 'prevSku'));
    }
...
//            echo '<pre>';            print_r($targetArr);exit;
        ...
        $view = view('specialProducts.showSelectedSKU', compact('targetArr'))->render();

...
class LatestProductsController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $prevData = ProductOffer::where('category', 2)->select('sku_data')->first();
        $prevSku = [];
        if (!empty($prevData)) {
...
//        exit;
        ...
        return view('latestProducts.index')->with(compact('request', 'qpArr', 'targetArr', 'prevSku'));
    }
...
//            echo '<pre>';            print_r($targetArr);exit;
        ...
        $view = view('latestProducts.showSelectedSKU', compact('targetArr'))->render();

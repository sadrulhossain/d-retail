...
class ProcessingOrderController extends Controller {
    ...
    public function index(Request $request) {
        ...
        //inquiry Details
        $targetArr = Order::whereIn('order.status', ['0', '2'])
//                ->leftJoin('invoice', 'invoice.order_id', 'order.id')
        ...
        $targetArr = $targetArr->select(DB::raw("CONCAT(users.first_name,' ',users.last_name) AS user_name")
                        , 'users.id as user_id', 'order.id as order_id', 'order.warehouse_id as warehouse_id'
                        , 'order.status', 'order.created_at', 'order.grand_total'
                        , 'retailer.name as retailer_name', 'order.order_no')
...
//
//        $deliveryDetailArr = DeliveryDetails::join('delivery', 'delivery.id', '=', 'delivery_details.delivery_id')
//                ->whereIn('delivery.order_id', $orderIdArr)
...
//                ->get();
        ...
        $deliveryInfo = Delivery::join('order', 'order.id', 'delivery.order_id')
                ->whereIn('order.id', $orderIdArr)
                ->select('delivery.order_id', 'delivery.id as delivery_id', 'delivery.bl_no'
                        , 'delivery.bl_date', 'delivery.payment_status', 'delivery.payment_mode')
        ...
        $deliveryDetails = DeliveryDetails::join('delivery', 'delivery.id', '=', 'delivery_details.delivery_id')
                ...
                ->select('delivery.order_id as order_id', 'delivery_details.sku_id'
                        , DB::raw('SUM(delivery_details.quantity) as quantity'))
                ->groupBy('delivery.order_id', 'delivery_details.sku_id')
                ->get();

        $deliveryDetailsArr = [];
        if (!empty($deliveryDetails)) {
            foreach ($deliveryDetails as $delivery) {
                $deliveryDetailsArr[$delivery->order_id][$delivery->sku_id] = $delivery->quantity;
            }
        }
//        print_r($deliveryDetailsArr);
//        exit;
        ...
        return view('processingOrder.index')->with(compact('request', 'qpArr', 'targetArr', 'orderArr'
                                , 'orderNoList', 'retailerList', 'srList', 'deliveryArr', 'deliveryDetailsArr'));
    }

    public function getDeliveryDetails(Request $request) {
        $loadView = 'processingOrder.showDeliveryDetails';
...
//          return Response::json(['success' => false, 'heading' => 'Validation Error', 'message' => $validator->errors()], 400);
        ...
        $deliveryInfo->order_id = $infoArr['order_id'];
//        $deliveryInfo->retailer_id = $infoArr['retailer_id'];
                    ...
                    if (!$insertDeliveryInfo) {//If failed to Insert in Consumption Details Table, Roll Back Insertion operation of Master Table
                        ...
                        if (!empty($productDetails)) {
                            ...
                            foreach ($productDetails as $item) {
                                ...
                                $updateWhStock = WarehouseStore::where('sku_id', $item->sku_id)
                                        ->where('warehouse_id', $whId)
                                        ->decrement('quantity', $item->quantity);
                                if (!$updateWhStock) {//If failed to Insert in Consumption Details Table, Roll Back Insertion operation of Master Table
                                    DB::rollback();
                                    return Response::json(['success' => false, 'heading' => 'Error', 'message' => __('label.DELIVERY_COULD_NOT_BE_SET')], 401);
                                }
                            ...
                            $invoiceInfo->order_id = $infoArr['order_id'];
                            ...
                            if ($invoiceInfo->save()) {
                                ...
                                if (!$detailsInsertStatus) {//If failed to Insert in Consumption Details Table, Roll Back Insertion operation of Master Table
                                    ...
                                    if ($request->payment_mode == '1') {
                                        ...
                                        $recieveInfo->order_id = $infoArr['order_id'];
                            ...
                            $deliveredQty = DeliveryDetails::join('delivery', 'delivery.id', '=', 'delivery_details.delivery_id')
                                            ->where('delivery.order_id', $request->order_id)
                                            ->select(DB::raw('SUM(delivery_details.quantity) as delivered_qty'))
                                            ->groupBy('delivery.order_id')
                                            ->first()->delivered_qty;

                            $orderQty = Order::join('order_details', 'order_details.order_id', 'order.id')
                            ...
                            } elseif (!empty($infoArr['mark_as_last_shipment'])) {
                                Order::where('id', $request->order_id)->update(['status' => '5']);
                            } else {
...
//                print_r($e->getMessage());
    ...
    public function printInvoice(Request $request, $id) {
        $loadView = 'processingOrder.printInvoice';
    ...
    public function markAsDelivered(Request $request) {
        if (empty($request->id)) {
            return Response::json(array('success' => false, 'heading' => 'Error', 'message' => __('label.INVALID_DATA_ID')), 401);
        }
        $update = Order::where('id', $request->id)->update(['status' => '5']);
        if ($update) {
            return Response::json(array('success' => true, 'heading' => 'Success', 'message' => __('label.ORDER_HAS_BEEN_DELIVERED_SUCCESSFULLY')), 200);
        } {

...
class CheckinLedgerReportController extends Controller {
...
//            $previousDelivery['amount'] = !empty($prevDeliveryInfo->total_amount) ? $prevDeliveryInfo->total_amount : 0;
            ...
            $previousBalance['quantity'] = $previousCheckIn['quantity'];
            $previousBalance['amount'] = $previousCheckIn['amount'];
        ...
        if ($request->view == 'print') {
            ...
            return view('report.checkinLedger.print.index')->with(compact('request', 'targetArr', 'totalAmount'
            ...
            $pdf = PDF::loadView('report.checkinLedger.print.index', compact('request', 'targetArr', 'totalAmount'
                                    , 'productList', 'konitaInfo', 'phoneNumber', 'fromDate', 'toDate', 'totalBalance'
                                    , 'totalDelivery', 'totalReturn', 'totalDamage', 'totalCheckIn', 'balanceArr'
                                    , 'ledgerArr', 'previousBalance'))
                    ->setPaper('a3', 'portrait')
                    ->setOptions(['defaultFont' => 'sans-serif']);
            return $pdf->download('checkin_ledger_report.pdf');
//            return $pdf->stream();
        } else {
            return view('report.checkinLedger.index')->with(compact('request', 'targetArr', 'totalAmount', 'productList'
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('admin/checkinLedgerReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }


        return Redirect::to('admin/checkinLedgerReport?generate=true&' . $url);
    }
